...
- **`--statusline-refresh <seconds>`**: with `--statusline`, actively polls the experimental usage API (min 30s, no more often than that regardless of how often Claude Code re-invokes the hook) to fill in any window (5h/7d) the passively-pushed official `rate_limits` doesn't have usable data for that refresh, without discarding a window official *does* have good data for. This is the mechanism for staying current across a sustained gap in Claude Code's own pushes. Separately, a single bare refresh (no `--statusline-refresh`, no official data at all) now falls back to the immediately-preceding refresh's numbers instead of going blank, though that alone doesn't survive more than one consecutive gap.
